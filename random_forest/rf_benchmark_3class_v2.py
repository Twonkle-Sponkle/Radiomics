...
for seed in SEEDS:
    ...
    for feature_set_name, feats in feature_sets.items():
        ...
        for train_idx, test_idx in cv.split(X, y):
            ...
            # per-class recall (0,1,2)
            class_rec = recall_score(y_test, preds, average=None, labels=[0,1,2])
...
for name in feature_sets.keys():
    ...
    sns.heatmap(
        ...
        fmt=".2f",
        cmap="Blues",
        xticklabels=["G1-2", "G3", "G4-5"],
        yticklabels=["G1-2", "G3", "G4-5"]
    )
...
for name in feature_sets.keys():
    ...
    print(f"  G1-2: {np.mean(class_vals, axis=0)[0]:.3f}")
    print(f"  G3  : {np.mean(class_vals, axis=0)[1]:.3f}")
    print(f"  G4-5: {np.mean(class_vals, axis=0)[2]:.3f}")
