...
OUTPUT_DIR = "benchmark_binaryclass"
...
for seed in SEEDS:
    ...
    for feature_set_name, feats in feature_sets.items():
        ...
        for train_idx, test_idx in cv.split(X, y):
            ...
            class_rec = recall_score(y_test, preds, average=None, labels=[0,1])
...
for name in feature_sets.keys():
    ...
    sns.heatmap(
        ...
        fmt=".2f",
        cmap="Blues",
        xticklabels=["G1-2", "G3-5"],
        yticklabels=["G1-2", "G3-5"]
    )
...
for name in feature_sets.keys():
    ...
    print(f"  G3-5: {np.mean(class_vals, axis=0)[1]:.3f}")
